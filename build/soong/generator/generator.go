...
func init() {
	android.RegisterModuleType("choose_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = chooseExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = chooseExpandVariables(ctx, dep_file)
	...
	cmd := chooseExpandVariables(ctx, String(g.properties.Cmd))
